...
const CoverPhoto = ({ handleClose, coverPhoto, username }) => {
    ...
    useEffect(() => {
        ...
        if (fiveToSix)
            setWidthHeight({ width: 500, height: 180 })

        if (fourToFive)
            setWidthHeight({ width: 400, height: 160 })

        if (ThreeToFour)
            setWidthHeight({ width: 300, height: 140 })

        if (TwoToThree)
            setWidthHeight({ width: 250, height: 120 })
    ...
    const showCroppedImage = useCallback(async () => {
        try {
            ...
            const { data } = await axios.post(`${process.env.REACT_APP_SERVER_URL}/user/${username}/cover_photo`, formData, {
                headers: {
